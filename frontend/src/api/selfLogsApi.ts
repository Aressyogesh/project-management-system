...
export const selfLogsApi = {
  ...
  // Learning logs
  getLearningLogs: (period?: string): Promise<LearningLog[]> =>
    apiClient.get('/learning-logs', { params: period ? { period } : undefined }).then((r) => r.data),

  createLearningLog: (dto: { period: string; topic: string; hours: number; description?: string }): Promise<LearningLog> =>
  ...
  // Innovation logs
  getInnovationLogs: (period?: string): Promise<InnovationLog[]> =>
    apiClient.get('/innovation-logs', { params: period ? { period } : undefined }).then((r) => r.data),

  createInnovationLog: (dto: { period: string; title: string; impact: string; type: string }): Promise<InnovationLog> =>
