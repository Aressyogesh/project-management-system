...
export function Topbar() {
  ...
  useEffect(() => {
    ...
    function handler(e: MouseEvent) {
      if (!gearRef.current?.contains(e.target as Node)) setShowGearMenu(false);
    }
    document.addEventListener('mousedown', handler);
    return () => document.removeEventListener('mousedown', handler);
  }, [showGearMenu]);
  ...
  return (
    <header className="flex items-center justify-between pl-16 lg:pl-6 pr-6 py-4 bg-white border-b border-gray-100 shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Settings gear — visible to all authenticated users */}
        <div className="relative" ref={gearRef}>
          ...
          {showGearMenu && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 py-1 overflow-hidden">
              ...
              {canAccessSettings && (
                <button
                  onClick={() => { setShowGearMenu(false); navigate('/settings/company'); }}
                  className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition flex items-center gap-2"
                >
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
