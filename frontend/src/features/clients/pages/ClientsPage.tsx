...
const emptyForm = (): FormState => ({ name: '', contactPerson: '', email: '', phone: '', address: '', businessUnitId: '' });
...
function ClientFormModal({ client, onClose, onSuccess }: FormModalProps) {
  ...
  const [form, setForm] = useState<FormState>(
    client
      ? { name: client.name, contactPerson: client.contactPerson, email: client.email, phone: client.phone ?? '', address: client.address ?? '', businessUnitId: client.businessUnit?.id ?? '' }
      : emptyForm(),
  );
...
export function ClientsPage() {
  ...
  const paged = clients.slice((safePage - 1) * pageSize, safePage * pageSize);
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{active} active · {clients.length - active} inactive</p>
          </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-gray-50 bg-gray-50/40 flex-wrap gap-3">
              <div className="flex items-center gap-2 text-xs text-gray-500">
                ...
                <span className="ml-1">
                  {clients.length === 0 ? '0' : `${(safePage - 1) * pageSize + 1}–${Math.min(safePage * pageSize, clients.length)}`} of {clients.length}
                </span>
              </div>
