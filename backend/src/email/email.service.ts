...
export class EmailService implements OnModuleInit {
  ...
  constructor(private config: ConfigService) {
    ...
    const backendUrl = this.config.get<string>('APP_BACKEND_URL') ?? 'http://localhost:3000';
    this.logoUrl = `${backendUrl}/public/pms-logo.png`;
  }

  onModuleInit(): void {
    const publicDir = join(process.cwd(), 'public');
    mkdirSync(publicDir, { recursive: true });
    const logoPath = join(publicDir, 'pms-logo.png');
    if (!existsSync(logoPath)) {
      writeFileSync(logoPath, Buffer.from(PMS_LOGO_BASE64, 'base64'));
    }
  }
